...
def test_classify_subdomain_equations(metadata_utils):
    ...
    result = metadata_utils.classify_problem(problem_text, "", MetadataProperties.SUBDOMAIN)
    
    print(f"Equations problem classifications: {result}")
    subdomain_list = result.get('subdomain', [])
...
def test_classify_subdomain_geometry(metadata_utils):
    ...
    print(f"Geometry problem classifications: {result}")
    subdomain_list = result.get('subdomain', [])
    assert len(subdomain_list) > 0
    subdomain = subdomain_list[0]
    assert "DOM_" in subdomain
...
def test_classify_subdomain_inequalities(metadata_utils):
    ...
    print(f"Inequalities problem classifications: {result}")
    subdomain_list = result.get('subdomain', [])
    assert len(subdomain_list) > 0
    # Check if any element contains "Inequalities" or matches DOM_Inequalities
    match_found = any("Inequalities" in item or item == "DOM_Inequalities" for item in subdomain_list)
    assert match_found 
