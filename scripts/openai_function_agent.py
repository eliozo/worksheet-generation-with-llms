...
class OpenaiFunctionAgent:
    ...
    weaviate_url = "NA"
    weaviate_api_key = "NA"
    gpt_model = DEFAULT_OPENAI_PROVIDER
    client = None
    #task_data = ""
    topic_list = []
    ...
    def __init__(self, openai_api_key, fuseki_url, fuseki_user, fuseki_password, weaviate_url, weaviate_api_key, task_data, provider=None):
        ...
        self.gpt_model = normalize_openai_provider(provider, warn=True)
...
ORDER BY ?grade"""
    ...
    def ask_to_find_topics(self, task_data): # List with topics
        ...
        if isinstance(merged_task.get('task'), dict):
            merged_task['task']['provider'] = self.gpt_model
